...
const { sendAppointmentNotificationToUser, sendAppointmentNotificationToStaff } = require('./notification.controller');

const createAppointment = catchAsync(async (req, res) => {
  const appointment = await appointmentService.createAppointment(req.body);

  const staffDetails = await userService.getUserById(appointment.preferredHairdresser);
  ...
  // Notify the staff
  await sendAppointmentNotificationToStaff({
    staffId: appointment.preferredHairdresser,
    type: 'new',
    appointmentDetails: appointment,
...
const updateAppointment = catchAsync(async (req, res) => {
  ...
  await sendAppointmentNotificationToStaff({
    staffId: appointment.preferredHairdresser,
    type: isUserAction ? 'user_updated' : 'staff_updated',
    appointmentDetails: appointment,
    userDetails: req.user,
    serviceDetails,
