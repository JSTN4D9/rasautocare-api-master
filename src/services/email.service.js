...
const generateClosingMessage = (type, isStaff) => {
  if (isStaff) {
...
const generateAppointmentEmailHtml = (type, appointmentDetails, recipientDetails, serviceDetails, isStaff) => {
  ...
  const emailTypes = {
    ...
    appointment_updated: {
      title: 'Appointment Updated - RAS Autocare',
  ...
  return `
    <div style="font-family: Arial, sans-serif; color: #333;">
      ...
      ${actionLink}
      <p>${generateClosingMessage(type, isStaff)}</p>
    </div>
  `;
};

const sendAppointmentEmail = async (type, to, appointmentDetails, recipientDetails, serviceDetails, isStaff = false) => {
  let subject;

  // Determine the subject based on the type
  switch (type) {
    case 'appointment_updated':
      subject = 'RAS Autocare Appointment Updated';
      break;
    case 'new_appointment':
      subject = 'New  Appointment';
      break;
    default:
      subject = `RAS Autocare Appointment ${type.charAt(0).toUpperCase() + type.slice(1)}`;
      break;
  }

  // Generate the HTML content
  const html = generateAppointmentEmailHtml(type, appointmentDetails, recipientDetails, serviceDetails, isStaff);
