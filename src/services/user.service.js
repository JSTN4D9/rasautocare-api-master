...
const getUserById = async (id, requester = null) => {
  ...
  // If requester is not provided, assume internal operation and allow access
  if (!requester) {
    return user;
  }
...
const updateUserById = async (userId, updateBody) => {
  ...
  // Handle role change to staff or back to user
  if (updatedUser.role === 'staff') {
    updatedUser.selectedUserId = userId; // Ensuring selectedUserId is set for staffs
  } else if (updatedUser.role === 'user') {
    updatedUser.selectedUserId = null; // Resetting fields when user is unassigned from staff
